...
.z.ac:{
    ...
    $[x[0] like "stream*";
        [if[not `token in key p:.api.prms x; :(2;.h.hn["401";`json;.api.errFormat "missing token field"])];
         a:p`token];
        a:x[1]`Authorization
    ];
    if[0=count a;:(2;.h.hn["401";`json;.api.errFormat "missing token"])];
    if[not 10h = type a; (2;.h.hn["401";`json;.api.errFormat "Invalid Authorization header"])];
    if[not a like "Bearer *"; (2;.h.hn["401";`json;.api.errFormat "Invalid Authorization header"])];
    res:.[.oauth.checkToken;(7_a;TENANT_ID;KDB_CLIENT_ID);{x}];
    $[.api.oauthActive and 10h = type res;
        (2;.h.hn["401";`json;.api.errFormat res]);
        (1;"example_user")
    ]
 };
